...
test('AI-defined matchmaking seed bank provides 100 anonymous heart-card answers', () => {
  ...
  assert.ok(SEED_ENTRIES.every((entry) => /性質：.+性別：.+年齡：.+主體：.+情緒：.+職業／身份：.+身材比例：.+臉蛋：.+特徵：.+服裝：.+服裝顏色：.+服裝材質：.+場景：.+配件／道具：.+寵物：.+光感：.+鏡位：.+構圖：.+畫風：.+動作／姿態：.+眼神：/.test(entry.localized.zh)));
  assert.ok(SEED_ENTRIES.every((entry) => /nature:.+gender:.+age bracket:.+subject\/action:.+emotion:.+occupation\/identity:.+body proportion:.+face:.+feature:.+outfit:.+outfit color palette:.+outfit material:.+scene:.+accessory\/prop:.+pet\/companion:.+lighting:.+camera angle\/viewpoint:.+composition structure:.+art style:.+action\/posture detail:.+gaze direction:/s.test(entry.englishPrompt)));
});
