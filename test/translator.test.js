...
test('text-to-image controls are split into guided setup tabs', () => {
  ...
  assert.match(indexSource, /Cosplay 也是非必填/);
  ...
  assert.match(indexSource, /outputSavePromptButton/);
  ...
  assert.match(appSource, /function refreshSelectableLanguage/);
  assert.match(appSource, /function refreshStaticSelectLanguage/);
  assert.match(appSource, /function refreshStaticUiLanguage/);
  assert.match(appSource, /STATIC_UI_TEXT/);
  assert.match(appSource, /setSelectOptions\(heartAge/);
  assert.match(appSource, /setSelectOptions\(intensity/);
  ...
  assert.match(styleSource, /--output-field/);
  assert.match(styleSource, /--output-text/);
  assert.match(styleSource, /--app-font/);
  assert.match(styleSource, /Noto Sans HK/);
  assert.match(styleSource, /Noto Sans JP/);
...
test('all customization selectors include AI judgment in the browser', () => {
  ...
  assert.match(appSource, /aiOption\.textContent = localizeShortText\('AI判斷'\)/);
