...
- Lower-pressure multi-page UX: Gmail-bound local login always starts from a visible login page, splits 設友/色友 modes, keeps Cosplay and every later configuration page optional so blank controls remain AI-decided; previous/next wizard pages expose only 2-3 controls, add a practical 業配 placement workflow, and further split character and scene/action controls into smaller substeps.
...
- High-contrast theme variables keep text, form fields, buttons, placeholders, and dropdown options visibly separated across all four color themes.

- Anonymous 配對所 community prototype: users save formal outputs as 悸動瞬間, optionally attach a compressed text-to-image thumbnail, publish selected moments as anonymous 心動卡, set desired 設友/色友 nature, gender, and adult age range, then reveal one of three matching cards with localized comparison text plus the complete English prompt; the pool also includes 100 AI-defined anonymous heart-card answers so matching is not limited to user uploads.
- Language switching now refreshes generated/local saved/community 心動卡 comparison text plus populated dropdown options and core static selectors into the selected interface language, paired with the unchanged English prompt output; 港語 is labeled as 粵語.
...
- 20 time-point presets with Chinese light/color/texture annotations, 50 emotion presets, 50 outfit-color presets, 50 outfit-material presets, 50 occupation presets, 50 body-proportion presets, age brackets through 60, 30 general body-feature presets plus 50 色友-only special identity/personality feature presets, outfit-integrity controls, 30 face presets, 20 count presets, 200 action/posture presets split into four modes, each with 25 normal and 25 adult-oriented options, and expanded per-character customization for gender, race, emotion, job, age, body, face, outfit, color, material, body feature, and outfit integrity when more than one character is selected.
