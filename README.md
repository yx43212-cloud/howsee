...
- Lower-pressure multi-page UX: Gmail-bound local login always starts from a visible login page, splits 設友/色友 modes, keeps Cosplay as the only required first-page prompt input, and lets every later configuration page be skipped so blank controls remain AI-decided; previous/next wizard pages expose only 2-3 controls, add a practical 業配 placement workflow, and further split character and scene/action controls into smaller substeps.
